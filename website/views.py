...
def home():
    ...
    # Basically when user hitting add(for Note) or getWeather (for Weather checking ) for askAI( for gemini) buttons. That is when you create a POST request to the server
    if request.method == "POST":
        ...
        if question:
            try:
                ...
                ai_answer = "The Gemini integration is currently disabled. This is a placeholder response."

                # Convert the AI response from Markdown to HTML
                ai_answer_html = markdown.markdown(ai_answer)
            except Exception as e:
                ai_answer_html = (
                    f"<p style='color:red;'>Error calling Gemini API: {str(e)}</p>"
                )
